...
<main class="flex-1 flex flex-col">
    <header
        class="flex items-center justify-between whitespace-nowrap border-b border-border-light dark:border-border-dark px-10 py-4 surface sticky top-0 z-10">
        <h1 class="txt txt-xl font-bold">Admin Dashboard</h1>
        <div class="flex items-center gap-4">
            <label class="flex flex-col min-w-40 !h-10 w-80">
                <div class="flex w-full flex-1 items-stretch rounded-lg h-full">
                    <div
                        class="text-primary flex border-none bg-primary/20 items-center justify-center pl-4 rounded-l-lg border-r-0">
                        <span class="material-symbols-outlined">search</span>
                    </div>
                    <input
                        class="input flex w-full min-w-0 flex-1 resize-none overflow-hidden rounded-lg focus:outline-0 focus:ring-0 border-none bg-primary/20 focus:border-none h-full placeholder:text-primary/70 px-4 rounded-l-none border-l-0 pl-2"
                        placeholder="Search features, users, content..." value="" />
                </div>
            </label>
            <button class="relative btn-ghost p-2">
                <span class="material-symbols-outlined">notifications</span>
                <div
                    class="absolute top-1.5 right-1.5 size-2.5 rounded-full bg-red-500 border-2 border-card-light dark:border-card-dark">
                </div>
            </button>
        </div>
    </header>
    <div class="flex-1 p-10">
        <div class="flex flex-wrap justify-between gap-4 pb-8">
            <div class="flex min-w-72 flex-col gap-2">
                <p class="txt txt-3xl font-bold">Good Morning,
                    <?php echo htmlspecialchars(explode(' ', $user['name'])[0]); ?></p>
                <p class="txt-2">You have <span class="font-bold text-red-500">3 new user feedback submissions</span>
                    today.</p>
            </div>
        </div>
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            ...
            <a href="<?php echo url('yoga'); ?>" class="card-feature col gap-3">
                <div class="w-full bg-center bg-no-repeat aspect-video bg-cover rounded-lg"
                    style='background-image: url("https://lh3.googleusercontent.com/aida-public/AB6AXuDn0PfnPcEKYLS33q971vE1JlizsniuP5Xx9Biw-eUNUJ3BJ11AG1U2_KgapqyLIiW9HIGiMS78XmETQMtf8kA5sAIekoeWha42CsmouWIT-c2yPEvf_bXOAh_sbpoC8-tIy1Rb765GEe-M8b-ExxEyRiP1a5xSFg67w1i9xmM78qXREB1Q37AEpLLoztPhRVOdIoiSRZKZJASCnSQtqft0xq-FoF4GlY2S-_fn4m3lS22WKd4mR4rGrOmj4sw7r4EDyQJuNf-TgGo");'>
                </div>
                <div>
